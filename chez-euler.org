...
(define (random-interval start width)
  (let ((A (random start))
	(W (random width)))
    (list A (+ A W))))

(define (list->test-sizes X)
  (map (lambda (x)
	 (random x))
       X))
#+end_src 
...
(define (e:193 n)
  ...
  (let loop ((count 0) (j 1))
    (if (> j lim)
        count
        (loop (+ count (* (vector-ref M j) (fx/ n (* j j))))
	      (1+ j)))))
...
segmented primes sieve, a totient sieve, and a moebius sieve.
...
An implementation of the [[https://en.wikipedia.org/wiki/Miller%E2%80%93Rabin_primality_test][Miller-Rabin primality test]], using 40 trial
rounds. We can confirm that
...
agrees with the basic eratosthenes sieve:
